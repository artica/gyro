...
SoftwareWire left( PinSDALeft, PinSCLLeft);
SoftwareWire center( PinSDACenter, PinSCLCenter);
SoftwareWire right( PinSDARight, PinSCLRight);

void setup()
{
	left.begin(); // join i2c bus (address optional for master)
	center.begin();
	right.begin();
}
